...
SYMBOL_LOT_SIZE_FALLBACK_MAP: Dict[str, float] = {
    "00700": 100.0,
    "0700": 100.0,
    "00700.HK": 100.0,
    "03690": 100.0,
...
class PositionManagementService:
            ...
            if not name:
                try:
                    ...
                    if isinstance(info_data, dict):
                        ...
                        lot_size = self._extract_lot_size(info_data)
                except Exception:
                    pass
            return {
    ...
    def _extract_lot_size(self, payload: Any) -> float:
        """Extract one-lot size from quote metadata."""
    ...
    def _lookup_symbol_lot_size(self, symbol: str, quote_symbol: str) -> float:
        """Resolve one-lot size from explicit symbol fallback mappings."""
        symbol_upper = str(symbol or "").strip().upper()
        quote_upper = str(quote_symbol or "").strip().upper()
        digits = "".join(ch for ch in symbol_upper if ch.isdigit())
        candidates = [quote_upper, symbol_upper]
        if digits:
            candidates.extend([digits, digits.zfill(4), digits.zfill(5)])
            candidates.extend([f"{digits.zfill(4)}.HK", f"{digits.zfill(5)}.HK"])
        for candidate in candidates:
            if candidate in SYMBOL_LOT_SIZE_FALLBACK_MAP:
                return SYMBOL_LOT_SIZE_FALLBACK_MAP[candidate]
        return 0.0
    ...
    def _resolve_lot_size(
        ...
        """Resolve board-lot size with quote data first and deterministic fallbacks."""
        quote_data = quote if isinstance(quote, dict) else {}
        quote_lot_size = self._extract_lot_size(quote_data)
        if quote_lot_size > 0:
            return quote_lot_size

        existing_lot_size = self._to_float(existing)
        if existing_lot_size > 0:
            return existing_lot_size

        mapped_lot_size = self._lookup_symbol_lot_size(symbol=symbol, quote_symbol=quote_symbol)
        if mapped_lot_size > 0:
            return mapped_lot_size
