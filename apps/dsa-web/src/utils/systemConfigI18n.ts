...
const categoryDescriptionMap: Partial<Record<SystemConfigCategory, string>> = {
  ...
  portfolio: '管理组合复盘总开关、持仓 JSON 和标签覆盖。',
...
const fieldTitleMap: Record<string, string> = {
  ...
  PORTFOLIO_ENABLED: '启用组合复盘',
  PORTFOLIO_INITIAL_CAPITAL: '组合初始资金',
  PORTFOLIO_TARGET_RETURN: '组合目标收益率',
  PORTFOLIO_HOLDINGS: '组合持仓 JSON',
  PORTFOLIO_STOCK_TAGS: '持仓标签覆盖 JSON',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  PORTFOLIO_ENABLED: '开启后，大盘复盘结束后会自动触发组合复盘，也可单独运行。',
  PORTFOLIO_INITIAL_CAPITAL: '组合复盘的初始资金基线，单位 CNY。',
  PORTFOLIO_TARGET_RETURN: '组合复盘目标收益率，0.30 表示 30%。',
  PORTFOLIO_HOLDINGS: '填写组合总资产、现金和 holdings 数组的 JSON，可使用多行格式。',
  PORTFOLIO_STOCK_TAGS: '按 ticker 覆盖 sector/style/beta 的 JSON 配置。',
