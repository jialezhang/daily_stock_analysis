...
/**
 * Watchlist component.
 * Shows recent followed analysis records and supports detail view + infinite loading.
 */
        ...
        {isLoading ? (
          ...
          <div className="space-y-1.5">
            {items.map((item) => (
              <div
                key={item.id}
                className={`history-item group ${selectedId === item.id ? 'active' : ''}`}
              >
                <button
                  type="button"
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 opacity-70 p-1 rounded hover:bg-white/10 text-muted hover:text-red-400 hover:opacity-100 transition-all"
                  title="删除记录"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
