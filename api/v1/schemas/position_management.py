...
class PortfolioReviewLatestResponse(BaseModel):
    """Latest persisted portfolio review snapshot."""

    found: bool = Field(..., description="Whether any portfolio review snapshot exists")
    message: str = Field("", description="Operation message")
    portfolio_review: Optional[Any] = Field(None, description="Latest portfolio review payload")


class PortfolioReviewHistoryResponse(BaseModel):
    """Historical persisted portfolio review snapshots."""

    found: bool = Field(..., description="Whether any portfolio review snapshot exists")
    message: str = Field("", description="Operation message")
    reviews: List[Any] = Field(default_factory=list, description="Portfolio review snapshot list")


class PortfolioReviewRunResponse(BaseModel):
    """Manual portfolio review run response."""

    success: bool = Field(..., description="Whether the run completed successfully")
    message: str = Field("", description="Operation message")
    report_preview: Optional[str] = Field(None, description="Preview of generated portfolio review")
    portfolio_review: Optional[Any] = Field(None, description="Latest portfolio review payload after run")
